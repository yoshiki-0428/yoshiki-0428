...
- **プロジェクト概要:**
  - 公共賭博の店舗情報などを公開するスマホアプリのAndroid、iOSアプリの改修案件を担当いたしました。改修内容としては、単純にアプリの画面開発だけでなく、新機能の追加の際に必要となるAPI項目の洗出〜作成依頼、アプリ側の通信〜画面表示までを担当いたしました。
...
- **プロジェクト詳細:**
  ...
  - チーム内で振り返り手法のKPTを利用し、開発効率の向上をしました。
...
- **プロジェクト概要:**
  - ゼロからの開発となりサービスの立ち上げを担当いたしました。社員のモチベーションを外部GmailやSlack、その他チャットツールを利用し社員のモチベーションをWatsonAPIにより判定させるWebアプリケーションです。
...
- **プロジェクト概要:**
  - 情報銀行としての役割を試験的に検証するためのPoC案件です。アプリの内容は登録者が詳細な個人情報以外の性格情報等を入力し、入力の代わりにオペレータなどが観光地の旅行プランを提案するAndroidアプリになります。
...
- **プロジェクト概要:**
  - 東京オリンピックのマスコットを募集するためのサイトのゼロから構築となります。Salesforceを利用した開発となります。
